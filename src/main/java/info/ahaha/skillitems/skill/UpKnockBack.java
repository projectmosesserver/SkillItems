...
public class UpKnockBack implements Skill {
    ...
    public UpKnockBack(int cost, int level) {
        this.cost = cost;
        this.level = level;
    }
    ...
    public void SkillActive(Player player, LivingEntity entity) {
        ...
        if (isUse(player.getInventory().getItemInMainHand())) {
            ...
            player.playSound(player.getLocation(), Sound.ENTITY_PLAYER_ATTACK_KNOCKBACK, 2, 1);
            pushAwayEntity(entity, -1.5, player);
        }
    }
    ...
    public void pushAwayEntity(Entity entity, double speed, LivingEntity victim) {
        ...
        entity.setVelocity(unitVector.multiply(speed).setY(1));
    }
}
